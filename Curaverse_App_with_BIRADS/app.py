...
def init_db():
    ...
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS birads_reports (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            patient_id INTEGER,
            source_file TEXT,
            file_kind TEXT,
...
.stat-icon.pink{background:#fdeaf3;color:#d52b88}
html.dark .stat-icon.pink{background:#3a1c30}
...
/* ---------- BI-RADS report ---------- */
.pill.birads-benign{background:#e6faf0;color:#1c9c62}
.pill.birads-watch{background:#fff1e2;color:#c9740f}
.pill.birads-suspicious,.pill.birads-malignant{background:#ffe3e3;color:#c0392b}
.pill.birads-unknown{background:#eef0f7;color:#5c6785}
html.dark .pill.birads-benign{background:#123a2b}
html.dark .pill.birads-watch{background:#402c14}
html.dark .pill.birads-suspicious,html.dark .pill.birads-malignant{background:#3a1c1c}
html.dark .pill.birads-unknown{background:#232849}
.reco-list{margin:8px 0 0;padding-left:18px}
.reco-list li{margin:3px 0;font-size:12.5px}
"""
...
NAV = [
    ...
    ("waveform", "BI-RADS Report", "/birads-report", True),
...
def patient_details(pid):
    ...
    <div class="head">
      ...
      <div style="display:flex;gap:8px">
        ...
        <a class="btn secondary" href="/birads-report/{pid}">{icon('waveform',15)} BI-RADS Report</a>
      </div>
    </div>
...
# ----------------------------------------------------------------------------
# BI-RADS Report Analyzer (OCR-based ultrasound report reader)
# ----------------------------------------------------------------------------
# Uses ultrasound_module.py (unmodified) to OCR an uploaded ultrasound report
# (image or PDF) and extract BI-RADS category, findings, risk level and
# recommendations. Imported lazily so app startup / the rest of Curaverse is
# never slowed down or blocked by loading the OCR engine.

_birads_module = None


def _get_birads_module():
    global _birads_module
    if _birads_module is None:
        import ultrasound_module as _bm
        _birads_module = _bm
    return _birads_module


def _birads_risk_class(birads):
    b = (birads or "").upper()
    if b in {"1", "2"}:
        return "birads-benign"
    if b in {"0", "3"}:
        return "birads-watch"
    if b in {"4", "4A", "4B", "4C", "5", "6"}:
        return "birads-suspicious"
    return "birads-unknown"


def render_birads_ui(patients, patient, last, upload_url, error_html=""):
    import json as _json
    ...
    if last:
        recos = _json.loads(last["recommendations"]) if last["recommendations"] else []
        reco_html = (
            "<ul class='reco-list'>" + "".join(f"<li>{escape(r)}</li>" for r in recos) + "</ul>"
            if recos else '<span class="muted">None listed</span>'
        )
        risk_class = _birads_risk_class(last["birads"])
        preview_html = (
            f'<img src="/uploads/{last["source_file"]}">' if last["file_kind"] == "image"
            else f'<div class="ph">{icon("file-text", 26)}<br>{escape(last["source_file"])}</div>'
        )
        result_html = f"""
        <div class="ip">
          <div class="box"><h3>Source Document</h3>{preview_html}<div class="meta">{patient["name"] if patient else "-"}</div></div>
          <div class="box" style="text-align:left">
            <h3 style="text-align:center">BI-RADS Result</h3>
            <p class="muted" style="margin:6px 0">Category<br><span class="pill {risk_class}">BI-RADS {escape(last["birads"])}</span></p><hr style="border-color:var(--line)">
            <p class="muted" style="margin:6px 0">Risk Level<br><b style="color:var(--text)">{escape(last["risk_level"])}</b></p>
            <p class="muted" style="margin:6px 0">Lymph Nodes<br><b style="color:var(--text)">{escape(last["lymph_nodes"])}</b>{(" (" + escape(last["axillary_region"]) + " axilla)") if last["axillary_region"] and last["axillary_region"] != "Unknown" else ""}</p>
            <div class="note">{icon('alert',14)}<span>OCR-based extraction — always confirm against the original radiology report.</span></div>
          </div>
          <div class="box" style="text-align:left">
            <h3 style="text-align:center">Findings</h3>
            <p class="muted" style="margin:6px 0">Breast Side<br><b style="color:var(--text)">{escape(last["breast_side"])}</b></p><hr style="border-color:var(--line)">
            <p class="muted" style="margin:6px 0">Lesion Type<br><b style="color:var(--text)">{escape(last["lesion_type"])}</b></p><hr style="border-color:var(--line)">
            <p class="muted" style="margin:6px 0">Size<br><b style="color:var(--text)">{escape(last["size"])}</b></p><hr style="border-color:var(--line)">
            <p class="muted" style="margin:6px 0">Location<br><b style="color:var(--text)">{escape(last["location"])} {escape(last["clock_position"]) if last["clock_position"] and last["clock_position"] != "Unknown" else ""}</b></p>
          </div>
          <div class="box" style="text-align:left">
        ...
        <div class="grid2" style="padding:0 15px 15px;margin-top:0">
          <div class="panel" style="margin-top:0;grid-column:1 / -1">
            <h3 style="margin:0 0 4px">Summary</h3>
            <p class="muted" style="margin:0">{escape(last["summary"])}</p>
          </div>
        </div>
        """
    else:
        result_html = """
        <div class="ip">
          <div class="box"><div class="ph">Waiting</div><div class="meta">No report analyzed yet</div></div>
          <div class="box"><div class="ph">Waiting</div></div>
          <div class="box"><div class="ph">Waiting</div></div>
          <div class="box"><div class="ph">Waiting</div></div>
        </div>
        """
    ...
    body = f"""
    <div class="head">
      <div><h1>BI-RADS Report Analyzer</h1><span class="muted">Dashboard › BI-RADS Report</span></div>
    </div>
    {error_html}
    <div class="panel" style="padding:0">
      <div class="steps">
        <div class="step done"><div class="circle">{icon('check',16)}</div><div><b>Select Patient</b><small>{patient["name"] if patient else "—"}</small></div></div>
        <div class="step {"done" if last else ""}"><div class="circle">2</div><div><b>Upload Report</b><small>Image or PDF ultrasound report</small></div></div>
        <div class="step {"done" if last else ""}"><div class="circle">3</div><div><b>OCR + Text Parsing</b><small>easyocr</small></div></div>
        <div class="step {"done" if last else ""}"><div class="circle">4</div><div><b>BI-RADS Extraction</b><small>Findings, risk, recommendations</small></div></div>
      </div>

      <form method="get" style="padding:0 15px 15px" onchange="this.submit()">
        <label>Patient</label>
        <select onchange="window.location='{upload_url.rsplit('/', 1)[0]}/'+this.value">{patient_options}</select>
      </form>

      {result_html}

      <form method="post" action="{upload_url}" enctype="multipart/form-data" class="upload-row" onsubmit="showLoader('Running OCR + BI-RADS extraction…')">
        <input type="file" name="report" accept=".png,.jpg,.jpeg,.bmp,.webp,.pdf" required>
        <button class="btn">{icon('upload',15)} Analyze Report</button>
      </form>
    </div>
    """
    return body
...
@app.route("/birads-report", defaults={"pid": None}, methods=["GET", "POST"])
@app.route("/birads-report/<int:pid>", methods=["GET", "POST"])
def birads_report(pid):
    import json as _json
    ...
    if request.method == "POST" and patient:
        ...
        if file and file.filename:
            suffix = Path(file.filename).suffix.lower()
            allowed = {".png", ".jpg", ".jpeg", ".bmp", ".webp", ".pdf"}
            if suffix not in allowed:
                session["birads_error"] = f"'{file.filename}' is not a supported image/PDF file."
                return redirect(request.path)
            name = "birads_" + uuid.uuid4().hex + suffix
            save_path = UPLOADS / name
            file.save(save_path)
            try:
                bm = _get_birads_module()
                result = bm.analyze_ultrasound(str(save_path))
            except Exception as e:
                save_path.unlink(missing_ok=True)
                conn.close()
                session["birads_error"] = f"Could not analyze this report: {e}"
                return redirect(request.path)
            conn.execute(
                """INSERT INTO birads_reports
                   (patient_id, source_file, file_kind, breast_side, lesion_type, size,
                    location, clock_position, lymph_nodes, axillary_region, birads,
                    risk_level, recommendations, summary, created_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    patient["id"], name, "image" if suffix != ".pdf" else "pdf",
                    result["breast_side"], result["lesion_type"], result["size"],
                    result["location"], result["clock_position"], result["lymph_nodes"],
                    result["axillary_region"], result["birads"], result["risk_level"],
                    _json.dumps(result["recommendations"]), result["summary"],
                    _dt.datetime.now().isoformat(timespec="seconds"),
                ),
            )
            conn.commit()
            conn.close()
            return redirect(f"/birads-report/{patient['id']}")

    last = None
    if patient:
        last = conn.execute(
            "SELECT * FROM birads_reports WHERE patient_id=? ORDER BY id DESC LIMIT 1",
            (patient["id"],),
        ).fetchone()
    conn.close()

    birads_error = session.pop("birads_error", None)
    if birads_error:
        error_html = f'<div class="note err" style="margin-bottom:15px">{icon("alert",14)}<span>{escape(birads_error)}</span></div>'

    body = render_birads_ui(
        patients, patient, last,
        f"/birads-report/{patient['id']}" if patient else "/birads-report",
        error_html,
    )
    return page(body, "BI-RADS Report")
...
def analytics():
    ...
    birads_count = conn.execute("SELECT COUNT(*) FROM birads_reports").fetchone()[0]
    reports_count = conn.execute("SELECT COUNT(*) FROM reports").fetchone()[0]
    ...
    <div class="cards">
      ...
      <div class="card"><div class="stat-top"><span class="stat-label">BI-RADS REPORTS ANALYZED</span><span class="stat-icon pink">{icon('waveform',16)}</span></div><div class="num" data-count="{birads_count}">0</div></div>
